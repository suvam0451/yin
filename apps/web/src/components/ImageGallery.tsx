...
function ImageGalleryRenderImage({url}: ImageGalleryRenderImageProps) {
  ...
  return <Box><img src={ImageUrl} alt={"missing image"} width={196} height={196}/></Box>
}
...
function ImageGallery() {
  ...
  // Queries
  const query = useQuery({queryKey: ['recent/image-prompts'], queryFn: api})
  useEffect(() => {
  ...
  return <Box bgColor={"#222"} my={8} py={4} px={8}>
    <Text pl={4} className={"darkmode_text_normal"} fontWeight={700} fontSize={"xl"}>Generated Recently:</Text>
    <Box display={"grid"} gridTemplateColumns={"1fr 1fr 1fr"}>
      {Data?.map((o: any, i: number) => <Box key={i} bgColor={"#333"} p={2} m={4}
                                             display={"flex"} flexDirection={"column"}
                                             borderRadius={"0.5rem"}>
        <Box flexGrow={1} display={"flex"} alignItems={"center"} flexDirection={"column"}>
        {o.imageGeneratePrompt?.results.length > 0
            && <ImageGalleryRenderImage url={o.imageGeneratePrompt?.results[0].mediaAsset.relativeUrl}/>}
        {o.imageGeneratePrompt.prompt && <Box mt={4}>
            <Text color={"#ffffff87"}>{o.imageGeneratePrompt.prompt}</Text>
        </Box>}
        </Box>
        <Box mt={4}>
        {o.user.username && <Text color={"#ffffff60"} fontSize={"sm"}>@{o.user.username} • {formatDistance(
            o.createdAt,
            new Date(),
            {addSuffix: true})
        }</Text>}</Box>
